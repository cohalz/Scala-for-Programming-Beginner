...
その一つが**リスト**です。  
リストを使うには、**Listとカッコを書いてその中に要素をコンマ(,)区切りで並べて書きます**。  
...
println("1 is pass")
```
##リストでのforはtoではなくuntilを使う
forを使うとき、リストの要素は5つなのに0 to 4と4を書いていました。  
...
##リストで使える便利なもの
リストはよく使うので、便利なものがあらかじめ定義されています。  
...
- **length**  
  ...
  リスト要素数が変われば自動的にlengthの値も変わります。  
  なのでforでリストの全要素を見る時によく使います。
...
- **fill**  
  ...
  中身が3個のリストで中身は全部1のリストを作ることが出来ます。  

  複数の整数値を入力させてリストにしたい場合にはreadIntを使います。
  ...
  今度は最初に要素がいくつあるのか入力させてリストを作ってみましょう。
