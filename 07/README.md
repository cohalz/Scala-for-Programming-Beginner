...
Listがこのように再帰的な構造をしていることはプログラムではとても重要で便利です。  
再帰構造については、詳しくは後々紹介していきます。  

##List自体は変更されない
大事なのが、このようなメソッドでList自体が変更されるというわけではないということです。  
例を見てみます。
```scala
val list = List(1, 2)
...
- **contains**  
  ...
  val list = List(1, 2, 3)
  if(list.contains(2)) {
    println("list contains 2") // listに2が含まれていたら実行される
  } else {
    println("list doesn't contains 2") // 含まれていない時のみ実行
  }
  ```
...
- **filter**  
  ...
  ```scala
  List(1, 2, 3, 4, 5).filter(_ % 2 == 0) // List(2, 4)が作られる
  ```
...
- **map**  
  ...
  ```scala
  List(1, 2, 3, 4, 5).map(_ % 3) // List(1, 2, 0, 1, 2)が作られる
  ```

##各要素を変数で扱う
filterやmapでは各要素をアンダーバーで扱いました。  
アンダーバーではなく、forで使ったiのような変数を中で使うことが出来ます。  
使いたい変数を書いて、=>のあとに実行したいものを書きます。  
以下のコードでは各要素がxで扱うことが出来るようになります。  
forの時のiのように、xはその前に書いてある変数ではないことを注意してください。
```scala
List(1, 2, 3, 4, 5).filter(x => x % 2 == 0) // List(2, 4)
```

では短いアンダーバーじゃなく変数を使う意味は何でしょう？  
実は要素をアンダーバーを使って取り出す場合、一回しか使えません。  
各要素を二乗したものに置き換えたListを作りたい場合、アンダーバーでは出来ません。
...
```scala
List(5, 6, 7, 8).map(x => if(x < 7) 2 * x else x) // List(10, 12, 7, 8)
```

##toやuntilの実態
forで使ったtoやuntilは実際にはListとよく似たRangeというものが作られています。
正確には、Listではありませんが、後にtoListと書くことでListに変更し、
上で書いたメソッドを使う事が可能です。
連続した数字を使いたいときに便利です。
```scala
...
```
```scala
(1 to 6).toList.filter(_ % 2 == 0) // List(2, 4, 6)が作られる。
```
##練習用課題
